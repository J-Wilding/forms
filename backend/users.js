...
// create a new user
// Notice we are using router.post here, because we are going to have our
// API inside of a module.  We're also using a route of / because we will
// configure the prefix for this router below in server.js.
router.post("/", async (req, res) => {
  ...
  try {
    ...
    // When we call the save method, the save hook will automatically salt and hash the password.
    await user.save();
...
// get logged in user
// When we use the middleware, validUser in this way, we are requiring users
// to be logged in before they can make this API call. Users who are not logged
// in will get a 403 error. Any user who is logged in will get their user document.
